        ...
        @foreach ($categories as $category)
            ...
            <div class="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                @foreach($products->where('category.name', $category) as $product)
                    <div class="bg-white shadow-lg rounded-lg overflow-hidden transform transition hover:scale-105 observer-item fade-in-up" style="animation-delay: 0.4s;">
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex items-center justify-between mt-4">
                                ...
                                <button id="pesan-btn-{{ $product->id }}" 
                                    onclick="addToCart('{{ $product->name }}', '{{ $product->description }}', {{ $product->price }}, '{{ $product->id }}')" 
                                    class="bg-yellow-400 text-white px-4 py-2 rounded-md font-semibold hover:bg-yellow-500 transition duration-300">
                                    Pesan Pickup
                                </button>
                            </div>
                        </div>
